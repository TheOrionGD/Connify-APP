...
export function ProfileSetupModal({ visible, onComplete }: ProfileSetupModalProps) {
  ...
  useEffect(() => {
    if (visible) {
      if (userProfile) {
        ...
        if (userProfile.medicalNotes) {
          try {
            ...
            if (parsed.guardian) {
              if (parsed.guardian.name) setGuardianName(parsed.guardian.name);
              if (parsed.guardian.phone) setGuardianPhone(parsed.guardian.phone);
              if (parsed.guardian.relationship) setGuardianRelation(parsed.guardian.relationship);
            }
      ...
      AsyncStorage.getItem('@connify_guardian_data').then(data => {
        if (data) {
          try {
            const parsedG = JSON.parse(data);
            if (parsedG.name) setGuardianName(prev => prev || parsedG.name);
            if (parsedG.phone) setGuardianPhone(prev => prev || parsedG.phone);
            if (parsedG.relationship) setGuardianRelation(prev => prev || parsedG.relationship);
          } catch (e) {}
        }
      });
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (guardianPayload.name || guardianPayload.phone) {
        try {
      ...
      const response = await profileApi.upsertProfile({
        ...
        firebaseUid: currentAuthUser?.uid || undefined,
