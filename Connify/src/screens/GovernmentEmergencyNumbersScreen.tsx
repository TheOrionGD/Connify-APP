...
export default function GovernmentEmergencyNumbersScreen({ navigation }: any) {
  ...
  const syncNumbersBackground = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // We won't interrupt the UI right now, it will apply on next load
      }
    } catch (e) {
  ...
  const renderCard = (title: string, number: string, icon: string, color: string) => (
    <TouchableOpacity style={styles.card} onPress={() => handleCall(number)}>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>GOVERNMENT EMERGENCY</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.container}>
          <View style={styles.titleSection}>
            <Text style={styles.mainTitle}>Local Authorities</Text>
            <Text style={styles.subtitle}>
              Based on your detected location region: {countryCode}. Tap any number to call immediately via your cellular network.
            </Text>
          </View>
