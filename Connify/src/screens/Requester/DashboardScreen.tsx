      ...
      <ScrollView contentContainerStyle={[styles.scrollContainer, { paddingBottom: tabBarHeight + 16 }]} showsVerticalScrollIndicator={false}>
        ...
        {!hasCompletedProfile && (
          <Animated.View entering={FadeInDown.duration(400).delay(250)} style={[styles.setupBanner, { backgroundColor: colors.surfaceContainerLowest, borderColor: colors.outline }]}>
            ...
            <Text style={[styles.setupBannerText, { color: colors.onSurfaceVariant }]}>
              You have basic mesh access. <Text style={{ color: '#EF4444', fontWeight: 'bold' }}>Warning:</Text> You will lose access to offline cellular request fallback during internet blackouts if your profile remains incomplete.
            </Text>
        ...
        {activeMode === 'need-help' ? (
          <>
            {currentState === 'searching' ? (
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                <GradientView
                  ...
                  style={[styles.activeSessionCard, { borderWidth: 1.5, borderColor: '#EF4444', borderRadius: 16, padding: 16 }]}
                >
                  <View style={styles.cardHeader}>
                    <View style={styles.cardHeaderTitle}>
                      <Icon name="radar" size={22} color="#FFFFFF" />
                      <Text style={[styles.activeSessionText, { color: '#FFFFFF' }]}>BROADCAST SEARCHING RESPONDERS</Text>
                    </View>
                    <View style={styles.liveBadgeWrapper}>
                      <View style={styles.pulseContainer}>
                        <PulseRipple color="#EF4444" size={40} />
                      </View>
                  ...
                  <Text style={{ fontFamily: 'WorkSans-Regular', fontSize: 13, color: '#FCA5A5', marginTop: 4, lineHeight: 18 }}>
                    Signal active. Transmitting anonymized location grid cell to nearby volunteer responders.
                  </Text>

                  <View style={styles.cardActions}>
                    <TouchableOpacity style={[styles.cardButton, { backgroundColor: '#EF4444', borderColor: '#EF4444' }]} onPress={() => navigation.navigate('Searching')}>
                      <Icon name="visibility" size={18} color="#FFFFFF" />
                      <Text style={[styles.cardButtonText, { color: '#FFFFFF' }]}>VIEW BROADCAST</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.cardButton, { backgroundColor: 'rgba(255, 255, 255, 0.1)', borderColor: 'rgba(255, 255, 255, 0.2)' }]} onPress={() => useEpisodeStore.getState().cancelRequest()}>
            ...
            ) : currentState === 'active' && userRole === 'responder' ? (
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                <GradientView
                  colors={['#064E3B', '#022C22']}
                  style={[styles.activeSessionCard, { borderWidth: 1.5, borderColor: '#34D399', borderRadius: 16, padding: 16 }]}
                >
                  <View style={styles.cardHeader}>
                    <View style={styles.cardHeaderTitle}>
                      <Icon name="handshake" size={22} color="#34D399" />
                      <Text style={[styles.activeSessionText, { color: '#34D399' }]}>ACTIVE RESPONDER ASSISTANCE</Text>
                    </View>
                    <View style={styles.liveBadgeWrapper}>
                      <View style={styles.pulseContainer}>
                        <PulseRipple color="#10B981" size={40} />
                      </View>
                      <View style={[styles.liveBadge, { backgroundColor: '#059669' }]}>
                        <Text style={styles.liveText}>ACTIVE</Text>
                      </View>
                    </View>
                  </View>

                  <Text style={{ fontFamily: 'WorkSans-Regular', fontSize: 13, color: '#A7F3D0', marginTop: 4, lineHeight: 18 }}>
                    Responding to {(category || 'Emergency Distress').toUpperCase()}. You are en-route / on scene helping the requester.
                  </Text>

                  <View style={styles.timerContainer}>
                    <View>
                      <Text style={[styles.timerLabel, { color: '#A7F3D0' }]}>SESSION WINDOW REMAINING</Text>
                      <Text style={[styles.timerText, { color: '#FFFFFF' }]}>{timerTextValue}</Text>
                    </View>
                    <View style={[styles.timerIconWrapper, { borderColor: '#34D399' }]}>
                      <Icon name="timer" size={28} color="#34D399" />
                    </View>
                  </View>

                  <View style={styles.cardActions}>
                    <TouchableOpacity style={[styles.cardButton, { backgroundColor: '#059669', borderColor: '#34D399' }]} onPress={() => navigation.navigate('EmergencyRequest')}>
                      <Icon name="radar" size={18} color="#FFFFFF" />
                      <Text style={[styles.cardButtonText, { color: '#FFFFFF' }]}>VIEW RADAR & MAP</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.cardButton, { backgroundColor: 'rgba(255, 255, 255, 0.15)', borderColor: 'rgba(255, 255, 255, 0.3)' }]} onPress={() => navigation.navigate('Feedback', { role: 'responder' })}>
                      <Icon name="check-circle" size={18} color="#FFFFFF" />
                      <Text style={[styles.cardButtonText, { color: '#FFFFFF' }]}>COMPLETE TASK</Text>
                    </TouchableOpacity>
                  </View>
                </GradientView>
              </Animated.View>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                ...
                >
                  ...
                  <View style={styles.cardActions}>
                    <TouchableOpacity style={[styles.cardButton, { backgroundColor: colors.surfaceContainerHigh, borderColor: colors.outline }]} onPress={() => navigation.navigate('EmergencyRequest')}>
                      <Icon name="visibility" size={18} color={colors.onBackground} />
                      <Text style={[styles.cardButtonText, { color: colors.onBackground }]}>VIEW EMERGENCY</Text>
                    </TouchableOpacity>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(400)} style={styles.statsGrid}>
          <StandardCard style={styles.statCard}>
            <Icon name="explore" size={22} color={colors.primary} />
            <View style={styles.statContent}>
              <Text style={[styles.statValue, { color: colors.onBackground }]}>500m</Text>
              <Text style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>LOCAL DISPATCH RADIUS</Text>
            </View>
          </StandardCard>
          ...
          <StandardCard style={styles.statCard}>
            ...
            <View style={styles.statContent}>
              <Text style={[styles.statValue, { color: colors.onBackground }]}>ACTIVE</Text>
              <Text style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>PEER MESH NETWORK</Text>
            </View>
          </StandardCard>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(500)}>
          <QuickSOSCard onPress={() => navigation.navigate('CreateRequest')} />
        </Animated.View>
      </ScrollView>
