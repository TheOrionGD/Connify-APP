...
import { openWhatsAppContact } from '../utils/phone';
...
export default function EmergencyContactsScreen({ navigation }: any) {
  ...
  const handleTriggerAll = (contact: EmergencyContact) => {
    ...
    const message = formatEmergencySMSMessage({
      alertType: 'EMERGENCY TRUSTED CIRCLE ALERT',
      userName,
      reason: 'Direct alert triggered to trusted circle contact',
  ...
  const handleWhatsApp = (contact: EmergencyContact) => {
    ...
    const message = formatEmergencySMSMessage({
      alertType: 'EMERGENCY TRUSTED CIRCLE ALERT',
      userName,
      reason: 'Direct alert sent via WhatsApp to trusted circle contact',
      relationship: contact.relationship || 'Emergency Contact',
      latitude,
      longitude,
    });
    openWhatsAppContact(contact.phone, message);
  };
        ...
        {contacts.map((contact) => (
          <View key={contact.id} style={[styles.card, { backgroundColor: colors.cardBackground, borderColor: colors.outline }]}>
            ...
            <View style={styles.cardActions}>
              ...
              <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#25D366' }]} onPress={() => handleWhatsApp(contact)}>
