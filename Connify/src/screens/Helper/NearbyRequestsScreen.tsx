...
export default function NearbyRequestsScreen({ navigation }: any) {
    ...
    const cleanup = socketService.onNewEpisode((ep: any) => {
      ...
      if (distanceMeters <= 10000) {
        ...
        setRequests(prev => {
          ...
          NotificationService.notifyNearbyResponderAlert(newReq.category, newReq.id).catch(() => null);
          return [newReq, ...prev];
        });
      }
    });

    const cleanupCancelled = socketService.onEpisodeCancelled(({ episodeId }: any) => {
      setRequests(prev => prev.filter(r => r.id !== episodeId));
      AsyncStorage.getItem('CACHED_NEARBY_FEED').then(cached => {
        if (cached) {
          try {
            const parsed = JSON.parse(cached) as HelpRequest[];
            const updated = parsed.filter(r => r.id !== episodeId);
            AsyncStorage.setItem('CACHED_NEARBY_FEED', JSON.stringify(updated)).catch(() => null);
          } catch (e) {}
        }
      });
    });
  ...
  const handleConfirmResponse = async () => {
    ...
    // Biometric verification before offering assistance on an episode
    const authenticated = await BiometricService.authenticateForEpisode('Offer Emergency Assistance');
    if (!authenticated) {
      return;
    }

    setHandshakeLoading(true);
    useRewardStore.getState().unlockBadge('SUPPORT_OFFERED');
    try {
      useEpisodeStore.getState().setUserRole('responder');
      useEpisodeStore.getState().setEpisodeId(selectedRequest.id);
      ...
      if (!blindedGridSigs || !helperValidationKey) {
        navigation.navigate('Handshake', { episodeId: selectedRequest.id, role: 'helper' });
        return;
      }
      ...
      socketService.offerSupport(selectedRequest.id, latitude!, longitude!, (res) => {
        const reqLat = res?.requesterLatitude;
        const reqLng = res?.requesterLongitude;
      ...
      >
        ...
        {hasValidLocation && (
          <LeafletMapView
            ...
            requests={requests.filter(r => r.latitude && r.longitude).map(r => ({
              id: r.id,
              latitude: r.latitude!,
