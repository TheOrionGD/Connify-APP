...
function runRealAnalysis() {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        // 2. Bloom Add
        const bloomA = new sharp_1.BloomFilter(1024, 4);
        const bloomB = new sharp_1.BloomFilter(1024, 4);
        t_bloomAdd.record(() => {
        ...
        let syndromesA = "";
        t_bchEncode.record(() => {
            syndromesA = sharp_1.SHARPHelper.generateSyndromes(bloomA.getBits());
        });
        // 4. Blind
        let blindedA = [];
        t_blind.record(() => {
            blindedA = cellsA.map(c => sharp_1.SHARPHelper.blindGridCell(sessionKey, c, "Alice"));
        });
        // 5. BCH Decode (Reconstruct)
        let reconstructedBitsA = new Uint8Array();
        t_bchDecode.record(() => {
            reconstructedBitsA = sharp_1.SHARPHelper.reconstruct(bloomB.getBits(), syndromesA);
        });
        // 6. Match
        t_match.record(() => {
            let matchCount = 0;
            const blindedB = cellsB.map(c => sharp_1.SHARPHelper.blindGridCell(sessionKey, c, "Bob"));
            for (const b of blindedB) {
                if (blindedA.includes(b))
                    matchCount++;
            }
        });
    }
    ...
    fs_1.default.writeFileSync(path_1.default.join(__dirname, '..', '..', '..', 'cpm_real_data_detailed.json'), JSON.stringify(fullData, null, 2));
