...
exports.SHARPHelper = exports.BloomFilter = exports.BCH157 = void 0;
...
function gfDiv(a, b) {
    if (a === 0)
        return 0;
    if (b === 0)
        throw new Error('Division by zero in GF(2^4)');
...
exports.BCH157 = BCH157;
// ── Bloom Filter ──────────────────────────────────────────────────────
class BloomFilter {
    constructor(sizeBits = 1024, hashCount = 4) {
...
class SHARPHelper {
    static generateSyndromes(bloomBits) {
        const syndromes = [];
        const getNext7Bits = (index) => {
            let val = 0;
            for (let i = 0; i < 7; i++) {
                const globalBitIndex = index * 7 + i;
                if (globalBitIndex >= 1024)
                    break;
    ...
    static blindGridCell(key, cell, role) {
        return sha256(`${key}:${cell}:${role}`);
    }
}
exports.SHARPHelper = SHARPHelper;
