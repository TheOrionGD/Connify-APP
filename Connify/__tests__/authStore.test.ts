...
// Stub @react-native-firebase/auth module factory
...
describe('authStore', () => {
  ...
  test('2. signInWithEmail success flow', async () => {
    ...
    // Configure auth() stub
    mockAuthInstance.signInWithEmailAndPassword.mockResolvedValue({ user: testUser });
  ...
  test('4. signUpWithEmail success flow', async () => {
    ...
    mockAuthInstance.createUserWithEmailAndPassword.mockResolvedValue({ user: testUser });
    ...
    expect(testUser.updateProfile).toHaveBeenCalledWith({ displayName: 'New User' });
  });
