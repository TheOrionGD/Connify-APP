...
export async function deviceRoutes(app: FastifyInstance): Promise<void> {
  ...
  /** Request a short-lived (60s) single-use challenge nonce for device verification. */
  app.post(
    '/challenge',
    {
    ...
    async (req, reply) => {
      return DeviceController.requestChallenge(reply);
    }
  );

  /** Device verification handshake. Consumes single-use challenge and verifies signature. */
  app.post(
    '/verify',
    { preHandler: authenticate },
    async (req, reply) => {
