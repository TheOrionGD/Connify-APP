...
export class LocationWatchdogService {
  ...
  public static async assertMandatoryGuardian(deviceId: string): Promise<void> {
    let count = await Guardian.countDocuments({ deviceId });
    if (count === 0) {
      // Auto-heal check: Check if Profile has guardian details stored in medicalNotes
      const profile = await Profile.findOne({ deviceId });
      if (profile && profile.medicalNotes) {
        try {
          const parsed = JSON.parse(profile.medicalNotes);
