...
export const CapsuleController = {
  async issue(input: IssueInput, reply: FastifyReply): Promise<void> {
    try {
      ...
      // Broadcast to the episode room that the capsule has been issued
      try {
        const io = getIO();
        const roomName = `episode:${input.episodeId}`;
        io.to(roomName).emit('capsule_issued', {
          episodeId: input.episodeId,
          status: 'matched',
          capsuleId: capsuleIdStr,
        });
        console.log(`📡 Broadcasted capsule_issued event to room ${roomName}`);
      } catch (err: any) {
        console.warn('⚠️ Failed to broadcast capsule_issued socket event:', err.message);
      }
