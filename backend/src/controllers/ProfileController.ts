...
export const ProfileController = {
  async upsertProfile(
    ...
    try {
      ...
      // Sync guardian data from medicalNotes into the Guardian collection.
      // The broadcast flow checks Guardian.countDocuments({ deviceId }) to enforce
      // mandatory guardian registration. Without this sync, guardian data saved via
      // onboarding (stored only in medicalNotes JSON) would never create a Guardian
      // document, causing GUARDIAN_REQUIRED errors on broadcast.
      if (data.medicalNotes) {
        try {
          const parsedNotes = JSON.parse(data.medicalNotes);
          const g = parsedNotes.guardian;
          const gName = g?.name || g?.fullName;
            ...
            await Guardian.findOneAndUpdate(
              { deviceId },
              {
                $set: {
                  deviceId,
                  userFullName,
                  fullName: gName || 'Emergency Guardian',
                  phone: gPhone || '',
          ...
          // medicalNotes is not valid JSON or missing guardian — skip silently
        }
      }
